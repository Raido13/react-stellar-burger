...
export const OrderInfo = ({updateTitle, orders, orderNumber}) => {
  const {burgerIngridients} = useSelector(selectorBurgerIngridients);
  const {status, ingredients: ingridientsIDs, name: orderName, createdAt} = orders.find(({number}) => orderNumber == number);
